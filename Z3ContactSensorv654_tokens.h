...
#define CREATOR_IAS_ZONE_CLUSTER_CLUSTER_REVISION_SERVER_1 0xB005
#define NVM3KEY_IAS_ZONE_CLUSTER_CLUSTER_REVISION_SERVER_1 ( NVM3KEY_DOMAIN_ZIGBEE | 0xB005 )
...
typedef uint16_t  tokType_ias_zone_cluster_cluster_revision_server;
#endif // DEFINETYPES
...
DEFINE_BASIC_TOKEN(IAS_ZONE_CLUSTER_CLUSTER_REVISION_SERVER_1, tokType_ias_zone_cluster_cluster_revision_server, 0x0001)
#endif // DEFINETOKENS
...
#define GENERATED_TOKEN_LOADER(endpoint) do {\
  ...
  if((endpoint) == 1 || ((endpoint) == EMBER_BROADCAST_ENDPOINT && epNetwork == curNetwork)) { \
    ...
    halCommonGetToken((tokType_ias_zone_cluster_cluster_revision_server *)ptr, TOKEN_IAS_ZONE_CLUSTER_CLUSTER_REVISION_SERVER_1); \
    emberAfWriteServerAttribute(1, ZCL_IAS_ZONE_CLUSTER_ID, ZCL_IAS_ZONE_CLUSTER_CLUSTER_REVISION_SERVER_ATTRIBUTE_ID, (uint8_t*)ptr, ZCL_INT16U_ATTRIBUTE_TYPE); \
  } \
} while(false)
  ...
  if ( endpoint == 1 ) { \
    if ( clusterId == 0x0500 ) { \
      ...
      if ( metadata->attributeId == 0xFFFD && 0x0000 == emberAfGetMfgCode(metadata) &&!emberAfAttributeIsClient(metadata) ) \
        halCommonSetToken(TOKEN_IAS_ZONE_CLUSTER_CLUSTER_REVISION_SERVER_1, data); \
    } \
  } \
} while(false)
